...
import { Card, Container, Dropdown, DropdownItem, DropdownMenu, DropdownToggle, Image, Navbar, Offcanvas, OffcanvasBody, OffcanvasHeader, OffcanvasTitle } from 'react-bootstrap';
...
const AgentMenu = () => {
  ...
  return <ul className="navbar-nav navbar-offcanvas-menu">
      {menuItems.map((item, idx) => {
      const Icon = item.icon;
      const activeMenuItem = item.url == pathname;
      return Icon && item.url && <li key={idx} className="nav-item">
              <Link className={clsx('nav-link items-center gap-1', {
      ...
      <Dropdown className="nav-item">
        <DropdownToggle as={Link} to="" className="arrow-none nav-link items-center gap-1" href="" id="dropdoanMenu" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
  ...
  return <section className="pt-4">
      <Container>
        <Card className="rounded-3 border p-3 pb-2">
          ...
          <button onClick={toggle} className="btn btn-primary w-100 d-block d-xl-none mt-2 items-center" type="button" data-bs-toggle="offcanvas" data-bs-target="#dashboardMenu" aria-controls="dashboardMenu">
